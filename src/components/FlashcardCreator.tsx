  ...
  const handleManualSave = async () => {
    ...
    try {
      const cardData = {
        ...
        back,
        description,
        title: description || 'Flashcards Gerais',
        subject: description || 'Geral',
        subjectName: description || 'Geral',
    ...
    try {
      const batch = generatedCards.map(card => {
        const data = {
          ...card,
          description,
          title: aiTopic || description || 'Flashcards de IA',
          subject: aiTopic || description || 'Geral',
          subjectName: aiTopic || description || 'Geral',
          ownerId: auth.currentUser!.uid,
          ownerName: auth.currentUser!.displayName || 'Estudante',
          isPublic,
          nextReview: serverTimestamp(),
